...
class Lepton3(Lepton):
    def __init__(self, spi_dev="/dev/spidev0.0"):
        ...
        self._xmit_buf = np.zeros(
            (self._msg_size * Lepton.ROWS * 4), dtype=np.uint8)
    ...
    def capture(self, data_buffer=None, debug_print=False):
        ...
        Lepton.capture_segment(
            self._handle, self._xmit_buf[:self._msg_size * Lepton.ROWS], self._msg_size, self._capture_buf[0])
        ...
        while (self._capture_buf[20, 0] & 0x00F0) != 0x10:  # are we at segment 1 yet?
            ...
            if (self._capture_buf[20, 0] & 0xFF0F) != 0x1400:
        ...
        for i in range(1, 4):
            Lepton.capture_segment(self._handle, self._xmit_buf[self._msg_size * Lepton.ROWS * i:self._msg_size * Lepton.ROWS * (
                i+1)], self._msg_size, self._capture_buf[i*Lepton.ROWS])
        ...
        if debug_print:
            ...
            for i in range(240):
                ...
                print("0x{0:04x} 0x{1:04x} : TTT={2} #{3:2} : crc={1}".format(
                    fid, crc, ttt, fnum))
            print("---")
