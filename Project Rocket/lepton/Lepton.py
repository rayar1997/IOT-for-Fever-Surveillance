...
class Lepton(object):
    ...
    def __init__(self, spi_dev="/dev/spidev0.0"):
        ...
        self._xmit_buf = np.zeros(
            (self._msg_size * Lepton.ROWS), dtype=np.uint8)
    ...
    def __enter__(self):
        ...
        ioctl(self._handle, SPI_IOC_RD_BITS_PER_WORD,
              struct.pack("=B", Lepton.BITS))
        ioctl(self._handle, SPI_IOC_WR_BITS_PER_WORD,
              struct.pack("=B", Lepton.BITS))

        ioctl(self._handle, SPI_IOC_RD_MAX_SPEED_HZ,
              struct.pack("=I", Lepton.SPEED))
        ioctl(self._handle, SPI_IOC_WR_MAX_SPEED_HZ,
              struct.pack("=I", Lepton.SPEED))
    ...
    def capture_segment(handle, xs_buf, xs_size, capture_buf):
        ...
        while messages > 0:
            if messages > Lepton.SPIDEV_MESSAGE_LIMIT:
                ...
                #print('message limit hit', messages)
            else:
                count = messages
            ...
            if ret < 1:
                raise IOError(
                    "can't send {0} spi messages ({1})".format(60, ret))
            messages -= count
    ...
    def capture(self, data_buffer=None, log_time=False, debug_print=False, retry_reset=True):
        ...
        if data_buffer is None:
            data_buffer = np.ndarray(
                (Lepton.ROWS, Lepton.COLS, 1), dtype=np.uint16)
        ...
        if debug_print:
            ...
            for i in range(Lepton.ROWS):
                ...
                fnum = fid & 0xFFF
                print("0x{0:04x} 0x{1:04x} : Row {2:2} : crc={1}".format(
                    fid, crc, fnum))
            print("---")

        if log_time:
            print("frame processed int {0}s, {1}hz".format(
